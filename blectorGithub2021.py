...
class Point:
    ...
    def __add__(self, other):
        ...
        otherPoint = other
        if (self.dimens == otherPoint.dimens):
            temp_add_list = [(s+o) for s, o in zip(self.coords, otherPoint.coords)]
            return Point(temp_add_list)
    def __repr__(self):
        return "Point({0})".format(str(self.coords))
...
class Rt:
    # 2021 update: added ranger parameter
    def __init__(self, basePoint, directionVector, asVector = False,ranger=list(range(-10,10))):
...
class Tile:
    def __init__(self, planeobj):
        self.planeobj = planeobj
        self.worldterrain = []
    def BuildWorldslist(self):
        # newly added in 2021: worlds terrain
        
        normvect = self.planeobj.normal_vector
        rot1 = np.array([[0,0,1],[1,0,0],[0,1,0]])
        ps1 = np.matmul(rot1, np.array(normvect.headc))
        ps1 = Point(np.ravel(ps1).tolist())
        
        rot2 = np.array([[0,1,0],[0,0,1],[1,0,0]])
        ps2 = np.matmul(rot2, np.array(normvect.headc))
        ps2 = Point(np.ravel(ps2).tolist())        
        ...
        for p_idx in range(len(ps)):
            curr_point = ps[p_idx]
            other_point = ps[1-p_idx]
            other_point_adj_list = []
            for i in list(range(-5,5,1)):
                other_point_adj = Point([c*i for c in other_point.coords])
                #rt1 = Rt(other_point_adj,Vector(curr_point.coords),ranger=list(range(-5,6,1)))
                #make(rt1)
                other_point_adj_list.append(other_point_adj)
            other_point_adj_arr.append(other_point_adj_list)
        
        traces = []
        for point_a in other_point_adj_arr[0]: # axis vector 1
            trace = []
            for point_b in other_point_adj_arr[1]: # axis vector 2
                new_point = point_a + point_b
                trace.append(new_point)
            traces.append(trace)
        self.worldterrain = traces        
        ...
        for vert_i in range(len(temp_obj_vertices) - 1):
            ...
            if ((not lastrowbool) and (not lastitembool)):
                try:
                    ...
                    #
                    bm.faces.new((vertB, vertA, vertC,vertD))
                except:
                    # original procedure
                    vertA = temp_obj_vertices[vert_i]
                    vertB = temp_obj_vertices[vert_i+1]
...
# updates for 2021:
# june 2nd, 2021
# see rt, added ranger parameter
# see new tile object
# changed addObject function to include tiles
# added import numpy
# added faces to addTerrain

#make(Rt([0,0,0],Vector([ 2,-1,0]))) # cannot set cz to 0, already being used
mvf3 = MVF("(1/2)*pow(@,2)+pow(!,2)")
#make(mvf3)
#print(mvf3.worldterrain[0][0])
#print(len(mvf3.worldterrain), len(mvf3.worldterrain[0]))


#pl1 = Plane(Equation([1, 2, 2, 0]))

#tl1 = Tile(pl1)
#make(tl1)
#print(tl1)
#print(tl1.worldterrain[0])
#print(len(tl1.worldterrain), len(tl1.worldterrain[0]))


#s ='''
# originally, only 1 point of intersection!
# notice dependent cols 1 and 3
# aug col does not work
tl1 = Tile(Plane(Equation([1, 2, 2, 0])))
tl2 = Tile(Plane(Equation([2, 3, 4, 0])))
tl3 = Tile(Plane(Equation([2, 1, 4, 0])))
...
s = '''
# but upon row reducing, now is a line!
tl4 = Tile(Plane(Equation([1, 0, 2, 0])))
tl5 = Tile(Plane(Equation([0, 1, 0, 0])))
tl6 = Tile(Plane(Equation([0, 0, 0, 0])))

make(tl4)
make(tl5)
make(tl6)      #'''

# intersectional arrow
# arrow of empty y is along the green axis, the y axis
#tl4 = Tile(Plane(Equation([1, 0, 0, 0])))
#tl5 = Tile(Plane(Equation([0, 0, 0, 0])))
#tl6 = Tile(Plane(Equation([0, 0, 1, 0])))
